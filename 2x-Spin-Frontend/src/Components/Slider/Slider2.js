...
const Slider = (props) => {
  ...
  const tokenSymbolImage = async (tokenMint, tokenType) => {
    if (tokenType == REWARD_TYPE_NFT) {
      ...
      // if (tokenMint.toString() === FRONK_MINT.toString()) {
      //   return { symbol: 'Bonk', image: "https://s2.coinmarketcap.com/static/img/coins/128x128/23095.png" };
      // }
      let addrStr = tokenMint.toBase58(); //"BXXkv6z8ykpG1yuvUDPgh732wzVHB69RnB9YgSYh3itW"; // tokenMint.toBase58();
      let apiurl = "https://api.solscan.io/account?address=" + addrStr;
  ...
  useEffect(() => {
    const func = async () => {
      if (wallet && isInitialized == false && isLoading == false) {
        setIsLoading(true);
        ...
        if (sData) {
          ...
          for (let i = 0; i < sData.count; i++) {
            ...
            tmpData[i].percent = Number(sData.ratioList[i]) / 10 ** PERCENTAGE_DECIMALS + "%";
  ...
  const updateLastUsersInfo = async () => {
    ...
    for (let i = 0; i < data.count; i++) {
      ...
      let rText = rUser + " Spin for " + payAmount + " $WUT and win " + rAmount + " " + rTokenName;
  ...
  return (
    <div className="container" style={{ marginTop: "50px" }}>
      {!adminInitFlag && (
        <div>
          <Modal isOpen={true} style={customStyles} contentLabel="Admin Init Confirm">
      ...
      <div>
        ...
        >
          <div className="openbox-container">
            <div
              className="openbox-wrap section-box-shadow"
              style={{ display: "block", margin: "0px", padding: "25px" }}
            >
              <button
                onClick={closeModal2}
                style={{ background: "transparent", top: "9px", right: "18px", fontSize: "38px", position: "absolute" }}
              >
                X
              </button>
              <h2 className="price-text-center mb-6" ref={(_subtitle) => (subtitle = _subtitle)}>
                {winnerItem?.price > 0 ? `You've won` : `You've burned`}
              </h2>
              <img src={winnerItem && winnerItem.image} style={{ maxWidth: "300px" }} alt="WUT_IMG" />
              <h3 className="price-text-center mt-6">{winnerItem?.price > 0 ? winnerItem.price : 444333} $WUT</h3>
            </div>
          </div>
        </Modal>
      </div>
      ...
      <div className="openbox-container">
        <div className="openbox-wrap section-box-shadow">
          <button
            ...
            title="Upgrade Mode"
          >
            Spin({payFronkAmount} $WUT)
          </button>
          {/* <div className="line-img">
            <img src={Line} alt="l"></img>
          </div> */}
          {claimEnabled ? (
            <button type="button" onClick={() => OnClickClaim()} className="openbox-btn btn-gradient">
              Claim
            </button>
          ) : (
            <></>
      ...
      <div
        ...
        className="banner-container bg-banner section-box-shadow"
      >
        {lastUsers.map((item) => (
          <div className="p-2" style={{ padding: "10px", fontSize: "20px" }}>
            <a href={`https://solscan.io/account/${item.user}`} target="_blank" rel="noreferrer">
              {item.sUser}
            </a>
            {item.reward >= 1000
              ? ` Spin for ${item.pay} $WUT and won ${item.reward} ${item.tokenName};`
              : ` Spin for ${item.pay} $WUT and burned 444333 ${item.tokenName};`}
          </div>
        ))}
      </div>
