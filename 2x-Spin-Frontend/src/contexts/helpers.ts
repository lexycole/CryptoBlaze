...
export const setPayInfo = async (wallet: any, connection: any, price: any, devFee: any, devWallet: PublicKey, burnFee: any, burnWallet: PublicKey) => {
  ...
  try {
    ...
    await program.rpc.setPayInfo(new anchor.BN(price * FRONK_DECIMAL), new anchor.BN(devFee * (10 ** PERCENTAGE_DECIMALS)), new PublicKey(devWallet), new anchor.BN(burnFee * (10 ** PERCENTAGE_DECIMALS)), new PublicKey(burnWallet),
      {
        accounts: {
...
export const spinWheel = async (wallet: any, connection: any) => {
  ...
  var fronkVault = await getTokenWallet(poolVaultPDA, FRONK_MINT);
  var fronkDev = await getTokenWallet(poolInfo.devWallet, FRONK_MINT);
  var fronkBurn = await getTokenWallet(poolInfo.burnWallet, FRONK_MINT);
  ...
  try {
      ...
      if (accInfo == null) {
        NotificationManager.error('You have no enough WUT token');
        return -1;
      }
  } catch (error) {
    NotificationManager.error('You have no enough WUT token');
    ...
    [
      {
        memcmp: {
          offset: 8,
          bytes: wallet.publicKey.toBase58() /*new PublicKey('7gWE7YFZeEBGhbYQ14kiJdgrHFYUgKEukaJjgNEkfd2D').toString()*/
        }
      }
    ]
  );
...
async function send(
  ...
  if (txHash != null) {
    ...
    if (res.value.err) {
      NotificationManager.error('WUT transaction failed');
      return null;
    }
    else {
      NotificationManager.success('WUT transaction confirmed');
    }
  } else {
    NotificationManager.error('WUT transaction failed');
  }
