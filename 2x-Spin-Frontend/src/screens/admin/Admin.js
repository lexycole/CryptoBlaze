...
function Admin() {
  ...
  const [fronkAmount, setFronkAmount] = useState(0);
        ...
        if (sData) {
          for (let i = 0; i < sData.count; i++) {
            ...
            let row = {
              ...
              type:
                sData.tokenTypeList[i] == REWARD_TYPE_NFT
                  ? "nft"
                  : sData.tokenTypeList[i] == REWARD_TYPE_SOL
                  ? "sol"
                  : "token",
            };
  ...
  const updateSettingInfo = async () => {
    try {
      ...
      if (sData) {
        ...
        setFronkAmount(pay_amount);

        pay_amount = sData?.solPrice?.toNumber() / LAMPORTS_PER_SOL;
        setPaymentSolAmount(pay_amount);

        let dev_fee = sData?.devFee?.toNumber();
        setDevFee(dev_fee / 10 ** PERCENTAGE_DECIMALS);

        let dev_wallet = sData?.devWallet?.toBase58();
        setDevWallet(dev_wallet);

        let burn_fee = sData?.burnFee?.toNumber();
        setBurnFee(burn_fee / 10 ** PERCENTAGE_DECIMALS);

        let burn_wallet = sData?.burnWallet?.toBase58();
        setBurnWallet(burn_wallet);
      }
    } catch (error) {
  ...
  const onSetRows = async (e) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      itemInfos.push({
        ...
        tokenType:
          rows[i].type == "nft" ? REWARD_TYPE_NFT : rows[i].type == "sol" ? REWARD_TYPE_SOL : REWARD_TYPE_TOKEN,
  ...
  const onSettingInfo = async () => {
    ...
    await setPayInfo(wallet, connection, fronkAmount, devFee, devWallet, burnFee, burnWallet);
    await updateSettingInfo();
    await closeModal();
  };
  ...
  return isAdmin == 2 ? (
    ...
    <div className="admin">
      ...
      >
        ...
        <div className="">
          <label htmlFor="">WUT Amount</label>
          <input
            onChange={(e) => setFronkAmount(e.target.value)}
            name="walletAddress"
            type="text"
        ...
        {isAdmin == 0 && (
          <>
            ...
            <div className="">
              <label htmlFor="">Burn Wallet</label>
              <input
          ...
          adminList.map((row, index) => {
            return (
              <div key={index} className="admin-flex">
                ...
                <button className="custom-btn remove-btn" onClick={() => onRemoveAdmin(row.toBase58())}>
                  Remove
                </button>
              </div>
            );
      ...
      <div className="admin-header">
        ...
        {isAdmin == 0 && (
          <button className="custom-btn add-btn" style={{ marginRight: "10px" }} onClick={() => openSuperAdminModal()}>
            Manage Admin
          </button>
        )}

        <button className="custom-btn add-btn" style={{ marginRight: "10px" }} onClick={() => openSettingModal()}>
          Setting
        </button>

        {isAdmin == 0 && (
          <>
            <button className="custom-btn add-btn" style={{ marginRight: "10px" }} onClick={() => onWithdraw(false)}>
              Withdraw token
            </button>

            <button className="custom-btn add-btn" style={{ marginRight: "10px" }} onClick={() => onWithdraw(true)}>
              Withdraw liquidity
            </button>
          </>
        )}
            ...
            <div key={index} className="admin-flex">
              <div className="admin-div">
                <div className="input-group">
                  ...
                  <select className="custom-input" onChange={(e) => OnChange(e, index)} name="type" value={row.type}>
              ...
              <button className="custom-btn remove-btn" onClick={() => RemoveRow(index)}>
                Remove
              </button>
            </div>
          );
